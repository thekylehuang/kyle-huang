...
const BlogCard = ({ post, index }: { post: Post, index: number }) => {
  return (
      <Link href={`/blog/${post.slug}`} scroll={false} >
        <motion.div 
        className='mt-4 hover:bg-zinc-200 hover:dark:bg-zinc-900 p-4 rounded-lg transition-colors flex justify-between flex-col md:flex-row'
        ...
        transition={{
          ...
          <h1 className={`${librebaskerville.className} text-neutral-950 dark:text-zinc-50 scroll-m-20 font-semibold tracking-tight text-lg lg:text-xl`}>{post.title}</h1>
...
const RecentBlogs = () => {
  ...
  return(
    <div className="w-full bg-zinc-50 dark:bg-neutral-950 border-b border-b-zinc-700">
      <div className="h-full mx-auto w-5/6 max-w-4xl text-neutral-950 dark:text-zinc-50 py-24">
        ...
        <Link href="/blog" className={`${geistmono.className} bg-zinc-50 dark:bg-neutral-950 underline text-md`}>
          <motion.p         
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }} 
          viewport={{ once: false }} 
